...
from .serializers import RegiterSerializer,JobSerializer,ApplicationSerializer
...
def basic_login(request):
    ...
    try:
       ...
       return Response({"user_id" : user.id , "username": user.username , "message":"Login Successfull"},status=status.HTTP_200_OK)
    except User.DoesNotExist:
        return Response({"messsage":"invalid Credentials"},status=status.HTTP_400_BAD_REQUEST)
